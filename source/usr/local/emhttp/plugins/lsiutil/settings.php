<?PHP
/* LSIUtil Settings Page — appears under Settings in the Unraid menu */
...
function checked(int $val): string {
    return $val ? 'checked' : '';
}
?>
  ...
  <form method="post">
    ...
    <div class="lu-s-card">
      ...
      <p style="font-size:12px;color:#555;margin:0 0 14px;">
        Temperature is always shown. Toggle the other panels below.
      </p>

      <label class="lu-toggle">
        <input type="checkbox" name="show_pcie" <?= checked((int)$cfg['SHOW_PCIE']) ?>>
        <span>PCIe Information</span>
        <small>Width &amp; speed in the Overview</small>
      </label>

      <label class="lu-toggle">
        <input type="checkbox" name="show_phy" <?= checked((int)$cfg['SHOW_PHY']) ?>>
        <span>PHY Health</span>
        <small>SAS link state &amp; error counters per port</small>
      </label>

      <label class="lu-toggle">
        <input type="checkbox" name="show_drives" <?= checked((int)$cfg['SHOW_DRIVES']) ?>>
        <span>Attached Drives</span>
        <small>SAS addresses, enclosure/slot, OS device names</small>
      </label>

      <label class="lu-toggle">
        <input type="checkbox" name="show_events" <?= checked((int)$cfg['SHOW_EVENTS']) ?>>
        <span>Event Log</span>
    ...
    <a class="lu-link" href="/Tools/LSIUtil">← Back to LSIUtil monitor</a>
